...
const appMiddlewareCliente = Router();
const appDTOData = Router();
const appValidateData = express();

appValidateData.use(async (req, res, next) => {
  try {
    const data = plainToClass(Cliente, req.body, { excludeExtraneousValues: true });
...
appMiddlewareCliente.use((req, res, next) => {
